...
- Enemies summoned by other enemies are also randomized (consistent replacement for each spawn)
...
- Known functional bosses:
  ...
  - CHAL_XION_M_Mann_01
  - CHAL_M_Scarlet_01
- Areas and game progression confirmed to be functional: Up until Maelstrom 1 (haven't tested beyond that)
...
- Minor issues:
  ...
  - Replacing Maelstrom with a different boss can result in unwinnable situations due to ammo shortage + no way to parry etc. => For now I am leaving these boss encounters untouched!
  - WLA_M_RoyalGuardFemale_01 (WLA_30_E_CharS_025) encounter no longer has battle music
...
- Things that go with the "shuffle bosses" setting:
  ...
  - "Fill empty boss slots with random duplicates" (useful if the player reduces the number of placeable bosses below the required amount)
